      ...
      //console.log(ind);
       ...
       if(onoff[ind]==false){  //각각의 1depth메뉴의 서브가 닫혀있냐??
        ...
        $(this).parents('.depth1').siblings('li').find('ul').slideUp('fast'); //자신을 제외한 모든 서브를 닫아라
         ...
         $('.depth1 i').attr('class','fa-solid fa-angle-down').css('color','#333');
         $('#gnb .depth1 h3 a').css('color','#333');
         $(this).find('i').attr('class','fa-solid fa-angle-up').css('color','#00a7ce');
         $(this).css('color', '#00a7ce');
         ...
         $(this).find('span').html('<i class="fa-solid fa-angle-down"></i>').css('color','#333');   
         $('#gnb .depth1 h3 a').css('color','#333');
       }
    });    
